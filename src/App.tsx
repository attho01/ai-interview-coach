...
export default function App() {
  ...
  const handleFetchQuestions = async () => {
    ...
    try {
      ...
      alert(`기출문제를 가져오는 중 오류가 발생했습니다: ${error?.message || "알 수 없는 오류"}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGenerateAnswer = async (question: string) => {
    ...
    try {
      ...
      alert(`답변을 생성하는 중 오류가 발생했습니다: ${error?.message || "알 수 없는 오류"}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      const dataUrl = await toPng(element, {
        pixelRatio: 2,
        backgroundColor: "#F8FAFC" // vibe-grid-bg color
      });
      ...
      // 남은 높이가 있다면 페이지를 추가하며 계속 이미지 슬라이싱
      while (heightLeft > 0) {
        // 이미지를 위로 끌어올려서(음수 Y좌표) 다음 영역이 보이도록 함
      ...
      pdf.save(`AI_면접코치_답변_${company}.pdf`);
    } catch (error: any) {
      console.error("PDF export failed:", error);
      alert(`PDF 내보내기 중 오류가 발생했습니다: ${error?.message || "알 수 없는 오류"}`);
    } finally {
            ...
            >
              ...
              <div className="vibe-card p-12 md:p-16 space-y-12">
                <div className="flex justify-between items-start gap-8">
                  ...
                  <div className="flex bg-brand-bg rounded-xl border border-black/[0.03] p-2 shrink-0">
                    <button
                      onClick={handleExportPDF}
                      disabled={isExporting}
                      className="flex items-center gap-2 px-5 py-3 hover:bg-white rounded-lg transition-colors font-bold text-sm text-brand-primary disabled:opacity-50"
                    >
                      {isExporting ? <Loader2 className="w-5 h-5 animate-spin" /> : <Download className="w-5 h-5" />}
                      PDF 다운로드
                    </button>
                    <div className="w-px bg-black/[0.05] mx-2" />
                    <button
